...
const [project, info, library, picker, reader, session] = await Promise.all([
  read('project.yml'),
  read('Supporting/Info.plist'),
  read('Sources/LibraryView.swift'),
  read('Sources/EPUBDocumentPicker.swift'),
...
assert.match(library, /EPUBDocumentPicker/, 'library must use the provider-compatible UIKit picker');
assert.match(picker, /forOpeningContentTypes: \[\.item\]/, 'picker must not let provider metadata disable EPUB files');
assert.match(picker, /asCopy: true/, 'picker must provide a readable local copy');
